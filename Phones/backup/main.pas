...
var
  Year, Month, Day: Word; // указание года, месяца и дня
  dt: TDateTime; //переменная для даты
  s: String;
begin
  Year:= 2023; //год
  Month:= 5;  // месяц
  Day:= 16;  // день
  dt:= EncodeDate(Year, Month, Day); //получение даты
  ShowMessage('Результат ' + (FormatDateTime('dddddd', dt))); // вывод даты
...
begin
ShowMessage ('Пешехонова Екатерина: Улица Пролетарский проспект дом 10 квартира 110');
ShowMessage ('Зайцев Глеб : Улица Пролетарский проспект дом 8 квартира 98');
ShowMessage ('Томин Егор : Улица Пролетарский проспект дом 5 квартира 76');
end;
